...
class ProteinTree:
    ...
    #print the tree for treePattern
    def printNHXTreePattern(self, f, root=None, withDist=True, withTags=False,  withAncGenesNames=True):
        NHX = {"Duplication": "D", "Bootstrap": "B", "taxon_name": "S", "duplication_confidence_score": "SIS", "dubious_duplication": "DD", "family_name": "F"}
        def rec(node):
...
def loadNHXTree(f):
    ...
    def convertNodeRec(tree, proteinTree, node):
        ...
        if "S" in tree.info[node]:
            ...
            #info["taxon_name"] = tree.info[node]["S"].replace(".", " ")
